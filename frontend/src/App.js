...
const Dashboard = () => {
  ...
  const handleChargingMode = async () => {
    ...
    try {
      ...
      if (response.data.is_real_account) {
        if (response.data.result === "WIN") {
          alert(`🚀 CHARGING MODE WIN on REAL ACCOUNT! ${response.data.trade.signal_type} ${response.data.trade.asset} for $${response.data.trade.amount.toFixed(2)}`);
        } else {
          alert(`⚠️ CHARGING MODE RESULT on REAL ACCOUNT: ${response.data.result} - ${response.data.trade.signal_type} ${response.data.trade.asset} for $${response.data.trade.amount.toFixed(2)}`);
        }
      }
  ...
  return (
    <DashboardLayout>
      ...
      {/* Trading Chart and Controls Grid */}
          ...
          <div className="mt-6 bg-white overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
                ...
                {dxyInfo && (
                  <div className="bg-gray-100 px-4 py-2 rounded-lg">
                    <div className="flex items-center space-x-2">
                      <span className="text-sm font-medium">DXY:</span>
                      <span className="text-lg font-bold">{dxyInfo.value.toFixed(2)}</span>
                      <span className={`text-sm font-medium ${dxyInfo.change_percent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {dxyInfo.change_percent >= 0 ? '+' : ''}{dxyInfo.change_percent.toFixed(2)}%
                        {dxyInfo.change_percent >= 0 ? (
                          <svg className="inline-block h-4 w-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5 mt-1 overflow-hidden">
                      <div 
                        className={`h-2.5 rounded-full ${dxyInfo.change_percent >= 0 ? 'bg-green-600' : 'bg-red-600'}`}
                        style={{ width: `${Math.min(Math.abs(dxyInfo.change_percent) * 10, 100)}%` }}
                      ></div>
                    </div>
                  </div>
                )}
              ...
              <div className="mt-4 overflow-auto" style={{ maxHeight: '250px' }}>
                {economicNews && economicNews.length > 0 ? (
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {economicNews.map((news, index) => (
                        <tr key={index} className={news.date === new Date().toISOString().split('T')[0] ? 'bg-blue-50' : ''}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {news.actual ? (
                              <span className={`font-medium ${
                                parseFloat(news.actual) > parseFloat(news.previous) ? 'text-green-600' :
                                parseFloat(news.actual) < parseFloat(news.previous) ? 'text-red-600' : ''
                              }`}>
                                {news.actual}
                              </span>
                            ) : (
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              ...
              <div className="mt-4 space-y-4">
                ...
                {/* Trading Symbol Selection with Search */}
                <div>
                  ...
                  <div className="mt-1 relative rounded-md shadow-sm">
                    <input
                      type="text"
                      className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pr-10 sm:text-sm border-gray-300 rounded-md mb-2"
